...
public class LevyWalkEntity extends TabuEntity
{
  ...
  final boolean researchCoverRatio = true; 
  //final Long seed = Long.valueOf(1531334499515691636L);
  //final Random random = new Random(this.seed);
  final Random random = new Random();
  final Double permissibleError = Double.valueOf(20.0); // 許容誤差
  final Double lambda = Double.valueOf(1.2); // スケーリングパラメータ,ラムダ
  final Double decimal = Double.valueOf(0.000001);
  ...
  protected void levyWalkStep()
  {
    ...
    for(Integer i = stepLength; 0 < i; i--) // stepLength分のLevyWalkを繰り返す
    {
      if(debug){
      ...
      while(possibleNeigbors.size() < 1) // 移動可能なエッジを見つけるまで繰り返す
      {
        for(Edge neigbor: neigbors)
        {
          ...
          if(debug) System.out.println("~~~~~~~~~ ~~~~~~~~~ error : permissibleError -> " + error + " : " + this.permissibleError);
  ...
  {
    // f(x) = a*x^k + o(x^k)
    Double aStepValue = Double.valueOf(Math.pow(this.getRandomValue(), (-1)*this.lambda)); // x^(-λ)
    while(aStepValue < 1) aStepValue = Double.valueOf(Math.pow(this.getRandomValue(), (-1)*this.lambda)); // 2x^(-λ);
    Integer aStepLength = Integer.valueOf(aStepValue.intValue());
    return aStepLength;
  }
